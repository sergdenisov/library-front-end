...
function Book({ title, description, authors, tags, year, link, countTotal, countCurrent }) {
  const countRemaining = countTotal - countCurrent;

  return (
    <article className="card">
      <div className="card-header">
        <h4 className="card-title">{link ? <a href={link} target="_blank">{title}</a> : title}</h4>
        <h5 className="card-subtitle text-gray">
          {authors.map((author, index) => `${author.text}${index !== authors.length - 1 ? ', ' : ''}`)}
        </h5>
